...
const Math = () => {
    const { color } = useStateContext(),
        exp1 = `$\\Delta y = v_{i} \\, sin\\left(\\alpha\\right) \\, t + \\frac{1}{2} g t^{2}$`,
        ...
        exp13 = `$y = tan\\left(\\alpha\\right) (x_{obs}) + \\frac{g \\,  (x_{obs})^{2} \\, sec^{2}\\left(\\alpha\\right)}{2 v_{i}^{2}}$`
    ...
    return (
        <div className="m-6 p-6 pt-4 bg-gray-800 rounded-xl h-fit">
            ...
            <div className="pt-5 text-white text-base font-normal text-justify">
                ...
                <br />
                Podemos ver de la ecuación de conservación de energía que la velocidad de salida del proyectil es directamente proporcional al cuadrado de la <span style={{color: color}}>compresión</span>, por lo que si buscamos tener la velocidad inicial máxima, la compresión del resorte deberá ser del 100%, es decir, 1 metro. Con esta información, podemos despejar la tercera equación para la velocidad inicial. Dado que <span style={{color: color}}>k</span> y <span style={{color: color}}>m</span> son valores que conocemos, podemos calcular el valor de la <span style={{color: color}}>velocidad incial</span>.
                <br /><br />
                <div className="flex justify-around text-lg">
                    <div className="inline-block text-black text-xl bg-white rounded-xl w-fit p-3">
                        <Latex>{exp4}</Latex>
                ...
                <br />
                Aún con esta redundancia, existirán casos en los que ninguna de las dos soluciones sea válida, así que en estos casos particulares repetiremos todo el procedimiento con una compresión del resorte ligeramente menor, hasta encontrar una trayectoria válida.
            </div>
        </div>
    )
}
